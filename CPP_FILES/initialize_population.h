...
void initialize_population( cmd_line &options, list<gene*> &trna_bank, int &trna_counter ) {
    ...
    //////////////////////////////////
        ...
        for ( int t = 0 ; t < options.start_count ; t ++ ) { 
            ...
            /// mutation rates :
            new_trna->somatic = options.somatic_rate * ((13.0 * (pow(new_trna->expression, 0.42))) + 1.3932) ;
            new_trna->germline = options.germline_rate * ((13.0 * (pow(new_trna->expression, 0.42))) + 1.3932) ;  
